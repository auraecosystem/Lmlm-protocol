...
-- Core Environment Helper Functions
...
runAbout _ = do outputStrLn "LTL24 data querying and verification system adapted for Lmlm-protocol, version 0.0.1."
...
runVerify _ = do env <- lift get
                 ...
                 let cgs :: [Game] -> [Spec] -> ProgramState
                     cgs [] _ = return ()
                     cgs (g:gs) ss = do outputStrLn ("Verifying dataset entry: " ++ (show $ gid g) ++ ".")
                                        cg g ss
                                        cgs gs ss
